print(paste0("Creating condition occurrence primary only at ", Sys.time()))

if(country_setting == "UK"){
cdm[["condition_occurrence_primary"]] <- 
  ...
  cdm[["condition_occurrence_primary"]] <- 
    cdm[["condition_occurrence"]] %>% 
    dplyr::filter(condition_type_concept_id == 32817|condition_type_concept_id ==32840) %>% 
    dplyr::compute()
}
...
cdm[["fracture"]] <- cdm[["denominator"]] %>% 
  dplyr::left_join(cdm[["condition_occurrence_primary"]], by = c("subject_id" = "person_id")) %>%
...
sub_output_folder <- here(output_folder, washout_period[[k]], "primary_rq2")
